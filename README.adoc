...
image:https://github.com/paneron/paneron/actions/workflows/macos-14.yml/badge.svg[alt="Build/release (macOS)",link="https://github.com/paneron/paneron/actions/workflows/macos-13.yml"]
...
NOTE: Development environment has been tested on macOS Sequoia and Ubuntu 18.

You’ll need the following installed:

* macOS: Xcode and the accompanying command-line tools
* Python 3.11 for node-gyp, which requires `distutils`
* Node 22.12+ (installation:
...
* Old Yarn (link:https://classic.yarnpkg.com/en/docs/install/[installation])
...
# Could be part of one-liner, e.g., when working on registry kit:
yarn upgrade --ignore-scripts @priboseinc/paneron-extension-kit \
&& yarn run build \
&& rsync -a --delete dist/ /path/to/paneron/dependencies-local/registry-kit-dist/ \
&& rsync -a --delete dist/ /path/to/some/extension/dependencies-local/registry-kit-dist/ \
&& rsync -a --delete dist/ /path/to/another/extension/dependencies-local/registry-kit-dist/
# after which, in Paneron directory
yarn upgrade --ignore-scripts registry-kit && yarn run dev
----
