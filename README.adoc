...
+
There is a pre-commit Git hook that compiles the app.
This helps us catch compilation errors before code reaches CI.
+
If for any reason you want to skip hooks, `git commit --no-verify` can be used.
...
++++
There is currently a commit-msg Git hook that validates commit messages with
`commitlint`.
