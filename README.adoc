...
* Optional: developer starts a feature (or fix) branch
  ...
  but it could be a major supported version branch[^versionbranch]
...
* When considered suitable, QA merges feature branch into its source branch
  ...
  onto the main[^versionbranch] branch

On each new commit in the main[^versionbranch] branch,
...
to the main[^versionbranch] branch, QA:
...
[^versionbranch]:
    ...
    NOTE: This process does not fully cover a situation where
    a new major version
